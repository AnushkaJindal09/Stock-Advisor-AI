...
# ---------- Lazy Load Model (safe for Railway) ----------
model = None
y_scaler = None
        ...
        for ticker in SORTED_TICKERS:
            try:
                ...
                if len(high) < 20 or len(low) < 20 or len(open_) < 20 or len(volume) < 20:
                    ...
                    continue
                result[ticker] = {"high": high, "low": low, "open": open_, "volume": volume}
...
def build_feature_matrix():
    ...
    print("last_20_days.npy updated!")
...
def predict():
    try:
        ...
        result = [{"company": name, "predicted_price": round(float(pred_actual[0][i]),2)}
...
def get_news():
    try:
        ...
        params = {"symbol": symbol, "from": week_ago.isoformat(), "to": today.isoformat(),
...
def get_stock():
    try:
        ...
        if quote:
            return jsonify({"symbol":symbol,"price":quote['lastPrice'],
                            "change":quote['change'],"percent_change":str(round(quote['pChange'],2))+"%"})
        else:
