...
function Prediction() {
  ...
  const priceDiffPercent = rangeMidPoint && livePrice
    ? (((rangeMidPoint - livePrice.price) / livePrice.price) * 100).toFixed(2) : null;
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-10">
        ...
        <div className="mb-6 md:mb-10">
          <div className="text-xs text-cyan-500 tracking-widest uppercase mb-2">AI Powered</div>
          <h1 className="text-2xl md:text-4xl font-bold text-white mb-2">Stock Price Predictor</h1>
          <p className="text-gray-400 text-sm">Quantitative Volatility Forecasting Engine Matrix</p>
        </div>

        {/* FIX 1: Grid items stretch implementation for equal length alignment */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">

          {/* Left — Company List */}
          <div className="lg:col-span-3 flex flex-col h-full">
            <div className="bg-gray-900 rounded-2xl border border-white/10 overflow-hidden flex flex-col flex-grow">
              <div className="px-4 py-3 border-b border-white/10">
                <span className="text-xs text-gray-400 uppercase tracking-widest">Select Company</span>
              </div>
              {/* Desktop view list matching the dynamic vertical panel container context */}
              <div className="p-2 flex lg:flex-col flex-row overflow-x-auto lg:overflow-x-visible lg:max-h-[620px] lg:overflow-y-auto gap-2 lg:gap-0 flex-grow">
                {COMPANIES.map(company => (
                  <div
                    key={company.symbol}
                    onClick={() => { setSelectedCompany(company.symbol); setPredictionResult(null); setError(''); }}
          ...
          <div className="lg:col-span-9 space-y-6 flex flex-col justify-between">
            <div className="space-y-6">
              ...
              <div className="bg-gray-900 rounded-2xl border border-white/10 p-4 md:p-6">
                ...
                {!selectedCompany && (
                  <div className="text-gray-600 text-center py-8">← Select a company to begin strategic analysis</div>
                )}
              ...
              >
                {loading ? (
                  <span className="flex items-center justify-center gap-3">
                    <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    Analyzing Structural Risk Bounds...
                  </span>
                ) : '🤖 Run Quantitative Prediction'}
              </button>
              ...
              {predictionResult && (
                <div className="bg-gray-900 rounded-2xl border border-white/10 overflow-hidden">
                  <div className="px-4 md:px-6 py-4 border-b border-white/10 flex items-center justify-between">
                    <span className="text-xs text-gray-400 uppercase tracking-widest">AI Target Boundary Metrics</span>
                    <span className="text-xs text-gray-600">T+1 Volatility Interval</span>
                  </div>

                  <div className="p-4 md:p-6">
                    {/* FIX 2: Restructured matrix grid layout. Range card given md:col-span-2 to expand layout space natively */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
                      
                      {/* Expanded Range Bounds Card Container */}
                      ...
                      <div className="bg-gray-800/50 rounded-xl p-3 md:p-4">
                        <div className="text-gray-400 text-xs mb-2">Last Swapped</div>
                        <div className="text-white text-base md:text-xl font-bold truncate">₹{livePrice?.price || '—'}</div>
                      </div>

                      {priceDiff && (
                        <div className="bg-gray-800/50 rounded-xl p-3 md:p-4">
                          <div className="text-gray-400 text-xs mb-2">Pivot Variance</div>
                          <div className={`text-base md:text-xl font-bold truncate ${isUp ? 'text-emerald-400' : 'text-red-400'}`}>
                            {isUp ? '+' : ''}{priceDiff}
                          </div>
                        </div>
                      )}
                    ...
                    <div className={`rounded-xl p-4 md:p-5 border ${isUp ? 'bg-emerald-900/20 border-emerald-500/30' : 'bg-red-900/20 border-red-500/30'}`}>
                      <div className="text-xs text-gray-400 mb-2 uppercase tracking-widest">Statistical Boundary Signal</div>
                      <div className={`text-xl md:text-2xl font-bold mb-1 ${isUp ? 'text-emerald-400' : 'text-red-400'}`}>
                        {isUp ? '📈 BIAS: Upward Distribution Zone' : '📉 BIAS: Downward Distribution Zone'}
                      </div>
                      <div className="text-gray-400 text-sm">
                        {isUp
                          ? 'Mathematical model metrics indicate structural accumulation above current equilibrium pricing levels.'
                          : 'Mathematical model metrics indicate structural distribution density vectors breaking below baseline supports.'}
                      </div>
                    </div>

                    <div className="mt-4 text-gray-600 text-xs text-center border-t border-white/5 pt-4">
                      ⚠️ SEBI Compliance Notice: This dashboard displays automated quantitative volatility models based entirely on structural deviation analytics. These calculations are strictly for statistical and data research validation parameters. This interface does not formulate guaranteed profit strategies, transactional blueprints, or investment advice.
                    </div>
                  </div>
                </div>
              )}
