...
export default function Portfolio() {
  ...
  const alertedRef = useRef({}); // track which alerts already sent browser notification
  ...
  const fetchNewssentiment = async (sym) => {
    try {
      ...
      // Simple sentiment — check headlines for negative/positive words
  ...
  const checkAlarms = async () => {
    ...
    for (const stock of currentPortfolio) {
      ...
      // Fetch ML prediction and news sentiment in parallel
      ...
      // --- SELL signals ---
      if (plPercent >= 8 && mlDirection === "down") {
        alertType = "SELL";
        urgency = "high";
        reasons = [`+${plPercent.toFixed(1)}% profit`, `ML predicts drop tomorrow`, newsSentiment === "negative" ? "Negative news" : ""].filter(Boolean);
        action = "Lock in your profits before it drops";
      } else if (plPercent >= 5 && mlDirection === "down" && newsSentiment === "negative") {
        alertType = "SELL";
        urgency = "high";
        reasons = [`+${plPercent.toFixed(1)}% profit`, "ML bearish", "Negative news"];
        action = "Strong sell signal — 3 indicators aligned";
      } else if (stock.targetPrice && currentPrice >= stock.targetPrice) {
        alertType = mlDirection === "up" ? "HOLD" : "SELL";
        urgency = "high";
        reasons = [`Target ₹${stock.targetPrice} reached!`, mlDirection === "up" ? "ML still bullish — consider holding" : "ML bearish — consider selling"];
        action = mlDirection === "up" ? "Target hit but ML says more upside" : "Target hit — good time to exit";
      } else if (plPercent <= -5 && mlDirection === "down") {
        alertType = "SELL";
        urgency = "high";
        reasons = [`${plPercent.toFixed(1)}% loss`, "ML predicts further drop", "Cut losses now"];
        action = "Stop loss recommended — avoid deeper loss";
      }
      // --- HOLD signals ---
      else if (plPercent >= 5 && mlDirection === "up") {
        alertType = "HOLD";
        urgency = "medium";
        reasons = [`+${plPercent.toFixed(1)}% profit`, "ML predicts further rise"];
        action = "Stay invested — more upside possible";
      } else if (plPercent <= -3 && mlDirection === "up") {
        alertType = "HOLD";
        urgency = "medium";
        reasons = [`${plPercent.toFixed(1)}% loss currently`, "ML predicts recovery tomorrow"];
        action = "Hold — ML expects price recovery";
      }
      // --- BUY MORE signals ---
      else if (plPercent <= -8 && mlDirection === "up" && newsSentiment === "positive") {
        alertType = "BUY";
        urgency = "medium";
        reasons = [`${plPercent.toFixed(1)}% dip`, "ML bullish", "Positive news"];
        action = "Potential buy-the-dip opportunity";
      }
      // --- WATCH signals ---
      else if (Math.abs(changeNum) >= 3) {
        alertType = "WATCH";
        urgency = "low";
        reasons = [`Price moved ${changeNum > 0 ? '+' : ''}${changeNum.toFixed(1)}% today`];
      ...
      setAlerts(prev => {
        ...
        if (exists) return prev.map(a => a.symbol === stock.symbol ? alertMsg : a);
        return [...prev, alertMsg];
      });

      // Browser notification — only once per symbol per session
      if (urgency === "high" && !alertedRef.current[stock.symbol] && Notification.permission === "granted") {
        alertedRef.current[stock.symbol] = true;
        new Notification(`${alertType} — ${stock.symbol}`, {
          body: `₹${currentPrice} | ${reasons[0]} → ${action}`,
          icon: "/favicon.ico"
        });
      }
    }
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="mb-8 flex items-end justify-between">
          ...
          {portfolio.length > 0 && (
            <button onClick={() => setShowChart(!showChart)} className="px-4 py-2 rounded-xl text-sm border border-white/10 text-gray-400 hover:text-white hover:border-cyan-500/50 transition">
          ...
          <div className="mb-8">
            ...
            <div className="space-y-3">
              {alerts.map((alert) => {
                ...
                return (
                  <div key={alert.symbol} className={`rounded-2xl border p-5 ${colors.bg} ${colors.border}`}>
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="text-gray-500 text-xs">₹{alert.currentPrice}</span>
                        ...
                        <div className="flex flex-wrap gap-2 mb-2">
                          {alert.reasons.map((r, i) => (
                            <span key={i} className="text-xs bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-gray-300">{r}</span>
                          ))}
                        </div>
                        <p className="text-sm text-white font-medium">→ {alert.action}</p>
                        <div className="flex items-center gap-4 mt-2 text-xs text-gray-600">
                          <span>ML: {alert.mlPrediction ? `₹${alert.mlPrediction} tomorrow` : 'N/A'}</span>
                          <span>News: {alert.newsSentiment}</span>
                          <span>{alert.time}</span>
                        </div>
                      </div>
                      <button onClick={() => dismissAlert(alert.symbol)} className="text-gray-500 hover:text-white ml-4 text-lg flex-shrink-0">✕</button>
                    </div>
                    <p className="text-xs text-gray-600 mt-2 italic">For educational purposes only — not financial advice</p>
                  </div>
                );
              })}
            </div>
                              ...
                              <div className="font-semibold text-white flex items-center gap-2">
                                ...
                                {stockAlert && (
                                  <span className={`text-xs px-1.5 py-0.5 rounded-md font-bold ${alertColors[stockAlert.type]?.badge}`}>
                                    {alertColors[stockAlert.type]?.icon} {stockAlert.type}
                                  </span>
                                )}
                              </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <span className={`px-2.5 py-1 rounded-lg text-xs font-bold ${colors.badge}`}>{alert.type}</span>
                          <span className="text-gray-400 text-sm">₹{alert.currentPrice}</span>
                          <span className={`text-sm font-semibold ${parseFloat(alert.plPercent) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                            {parseFloat(alert.plPercent) >= 0 ? '+' : ''}{alert.plPercent}% from buy
                          </span>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-3">
                          {alert.reasons.map((r, i) => (
                            <span key={i} className="text-xs bg-white/5 border border-white/10 rounded-lg px-3 py-1 text-gray-300">{r}</span>
                          ))}
                        </div>

                        {/* Action */}
                        <p className="text-white font-semibold text-sm mb-2">→ {alert.action}</p>

                        {/* Meta info */}
                        <div className="flex items-center gap-4 text-xs text-gray-500">
                          {alert.mlPrediction && <span>ML: ₹{alert.mlPrediction} tomorrow ({alert.mlDirection})</span>}
                          <span>News: {alert.newsSentiment}</span>
                          <span>Updated: {alert.time}</span>
                        </div>
                      </div>
                      <button onClick={() => dismissAlert(alert.symbol)} className="text-gray-500 hover:text-white ml-4 text-xl flex-shrink-0">✕</button>
                    </div>
                    <p className="text-xs text-gray-600 mt-3 italic border-t border-white/5 pt-2">For educational purposes only — not financial advice</p>
                  </div>
                );
              })}
            </div>
