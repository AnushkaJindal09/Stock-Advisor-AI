...
function Home({ isLoggedIn: propsIsLoggedIn, onLogout: propsOnLogout, setIsAuthOpen: propsSetIsAuthOpen }) {
    const [menuOpen, setMenuOpen] = useState(false);
    const [isAuthOpen, setIsAuthOpen] = useState(false);
    
    // 100% Accurate Login Check: Pehle App.jsx ka state dekho, nahi toh direct LocalStorage check karo
    const isLoggedIn = propsIsLoggedIn !== undefined ? propsIsLoggedIn : !!localStorage.getItem('token');
    
    const navigate = useNavigate();

    // STRICT NAVIGATION LOGIC: Sirf aichat aur portfolio par hi login maangega!
    const handleNavigation = (targetPath) => {
        // Agar page Home, Markets, Forecast (graph), ya News hai -> TOH KOI LOGIN NAHI CHAHIYE!
        if (
            targetPath === "/home" || 
            targetPath === "/" || 
            targetPath === "/markets" || 
        ...
        // Agar premium pages hain (/aichat ya /portfolio) aur login nahi hai -> Tabhi popup aayega
        if (!isLoggedIn) {
            if (propsSetIsAuthOpen) {
                propsSetIsAuthOpen(true); // App.jsx wala global modal khulega taaki cross button har jagah chale
            } else {
                setIsAuthOpen(true); // Local fallback modal
            }
        } else {
            navigate(targetPath); // Agar logged in hai toh mast aage bhej do
        }
        setMenuOpen(false);
    };
        ...
        <div className="w-full px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex space-x-8 text-sm font-medium tracking-wide text-gray-300 items-center">
            ...
            {isLoggedIn ? (
              ...
              <button 
                onClick={() => propsSetIsAuthOpen ? propsSetIsAuthOpen(true) : setIsAuthOpen(true)}
        ...
        {menuOpen && (
          <div className="md:hidden bg-black/90 border-t border-white/10 px-6 py-4 flex flex-col gap-4">
            ...
            {isLoggedIn ? (
              ...
              <button 
                onClick={() => { propsSetIsAuthOpen ? propsSetIsAuthOpen(true) : setIsAuthOpen(true); setMenuOpen(false); }}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[90vh] px-6 text-center">
        ...
        <div className="relative z-10 max-w-3xl mx-auto w-full">
          ...
          <button 
            ...
            className="px-8 py-3 rounded-xl font-semibold text-md bg-gradient-to-r from-blue-600 via-cyan-600 to-emerald-600 hover:opacity-90 transition shadow-lg shadow-cyan-500/20 cursor-pointer text-white border-none"
          >
            Start AI Chat
          </button>
      ...
      <div className="relative z-10 px-4 md:px-8 max-w-5xl mx-auto mb-16 md:mb-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} onClick={() => handleNavigation(feature.link)}>
...
export default function Myhome({ isLoggedIn, onLogout, setIsAuthOpen }) {
    return (
        <Home 
...
function Home({ isLoggedIn: propsIsLoggedIn, setIsAuthOpen: propsSetIsAuthOpen }) {
    // 100% Accurate Login Check: Pehle App.jsx ka state dekho, nahi toh direct LocalStorage check karo
    const isLoggedIn = propsIsLoggedIn !== undefined ? propsIsLoggedIn : !!localStorage.getItem('token');
    const navigate = useNavigate();

    // STRICT CARD NAVIGATION LOGIC: Features cards se click karne par sahi filters chalenge
    const handleFeatureCardNavigation = (targetPath) => {
        if (
            targetPath === "/home" || 
            targetPath === "/" || 
            targetPath === "/markets" || 
        ...
        // Sirf premium pages hain (/aichat ya /portfolio) aur login nahi hai -> Tabhi popup aayega
        if (!isLoggedIn) {
            if (propsSetIsAuthOpen) {
                propsSetIsAuthOpen(true); // App.jsx wala global modal khulega taaki cross button chal sake
            }
        } else {
            navigate(targetPath); // Agar logged in hai toh mast aage bhej do
        }
    ...
    <div className="relative text-white min-h-screen">
      ...
      {/* FIXED: Purana local navbar block yahan se safely clean kar diya hai, taaki double navbar na aaye */}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[90vh] px-6 text-center">
        ...
        <div className="relative z-10 max-w-3xl mx-auto w-full">
          ...
          <button 
            onClick={() => handleFeatureCardNavigation('/aichat')} 
            className="px-8 py-3 rounded-xl font-semibold text-md bg-gradient-to-r from-blue-600 via-cyan-600 to-emerald-600 hover:opacity-90 transition shadow-lg shadow-cyan-500/20 cursor-pointer text-white border-none"
          >
            Start AI Chat
          </button>
      ...
      <div className="relative z-10 px-4 md:px-8 max-w-5xl mx-auto mb-16 md:mb-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} onClick={() => handleFeatureCardNavigation(feature.link)}>
              <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-4 md:p-6 hover:border-cyan-500/40 hover:bg-white/8 transition-all duration-300 h-full cursor-pointer group">
                <div className="text-2xl md:text-3xl mb-3">{feature.icon}</div>
                <div className="text-white font-semibold text-xs md:text-sm mb-2 group-hover:text-cyan-400 transition">{feature.title}</div>
                <div className="text-gray-500 text-xs leading-relaxed hidden md:block">{feature.desc}</div>
